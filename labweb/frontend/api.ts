...
{
    const response = await fetch(API_RECIEVE_OBJECT_POINTS_URL, { method: "POST", body: JSON.stringify(data)})
    console.log(response)
}

export async function open_experiment(data : string)
{
    const response = await fetch(API_RECIEVE_OBJECT_POINTS_URL, { method: "POST", body: data})
    console.log(response)
}
